...
VedioPlayer::VedioPlayer()
{
    ...
    m_videoState = VideoState(); // 初始化 VideoState 为全新状态
}
//时间补偿函数--视频延时
...
void VedioPlayer::SendGetOneImage(QImage& img)
{
    emit SIG_getOneImage(img); //发送信号
}

void VedioPlayer::play()
{
...
//跳转
void VedioPlayer::seek(int64_t pos) //精确到微秒
{
...
void VedioPlayer::stop(bool isWait)
{
    qDebug()<< "VideoPlayer::Stop111";
    m_videoState.quit = 1;
...
PlayerState VedioPlayer::playerState() const
{
...
void VedioPlayer::run()
{
    ...
    if (videoStream == -1 && audioStream == -1) {
        printf("Couldn't find any audio or video stream\n");
    ...
    // 如果是纯音频，先发送一个默认的音频播放图像
    if (videoStream == -1 && audioStream != -1) {
        QImage audioImage(800, 600, QImage::Format_RGB32);
        audioImage.fill(Qt::black);
        QPainter painter(&audioImage);
        painter.setPen(Qt::white);
        QFont font;
        font.setPointSize(24);
        painter.setFont(font);
        painter.drawText(audioImage.rect(), Qt::AlignCenter, "纯音频播放中...");
        painter.end();
        SendGetOneImage(audioImage);
    }
    ...
    while(1)
    {
        ...
        不是包的数量 */
        //这个值可以稍微写大一些
        if( m_videoState.audioStream != -1 && m_videoState.audioq != nullptr && m_videoState.audioq->size >
                MAX_AUDIO_SIZE ) {
            SDL_Delay(10);
            continue;
        }
        if ( m_videoState.videoStream != -1 && m_videoState.videoq != nullptr && m_videoState.videoq->size >
             MAX_VIDEO_SIZE) {
            SDL_Delay(10);
            continue;
        }
        ...
        if( m_videoState.seek_req )
            ...
            if (av_seek_frame(m_videoState.pFormatCtx, stream_index, seek_target,
                ...
                if (m_videoState.audioStream >= 0 && m_videoState.audioq != nullptr) {
                ...
                if (m_videoState.videoStream >= 0 && m_videoState.videoq != nullptr) {
                    AVPacket *packet = (AVPacket *) malloc(sizeof(AVPacket)); //分配一个 packet
                    av_new_packet(packet, 10);
                    strcpy((char*)packet->data,FLUSH_DATA);
        ...
        DelayCount = 0;
        //生成图片
        if (m_videoState.videoStream != -1 && packet->stream_index == m_videoState.videoStream && m_videoState.videoq != nullptr)
        {
            packet_queue_put(m_videoState.videoq, packet);
        }
        else if (m_videoState.audioStream != -1 && packet->stream_index == m_videoState.audioStream && m_videoState.audioq != nullptr)
        {
...
void VedioPlayer::setFileName(const QString &newFileName)
{
...
//获取当前时间
double VedioPlayer::getCurrentTime()
{
    if (m_videoState.audioStream != -1) {
...
//获取总时间
int64_t VedioPlayer::getTotalTime()
{
