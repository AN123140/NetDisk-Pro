...
/*-------------数据库信息-----------------*/
#define _DEF_DB_NAME    "NewNetDisk"
...
struct FileInfo
{
    ...
    FileInfo():fid(0) , size(0),fileFd( 0 ) , pos(0){
...
struct STRU_GET_FILE_INFO_RS
{
    ...
    void init() {
        type = _DEF_PACK_GET_FILE_INFO_RS;    // 设置类型为获取文件信息响应
        count = 0;                            // 计数器清零
        memset(dir, 0, sizeof(dir));          // 清空dir数组/结构体
    }
    PackType type;
    char dir[_MAX_PATH_SIZE];
    int count;
...
//分享文件请求：包含谁分享什么目录下面的哪些文件（文件id数组）分享时间
...
/////////////////////在线播放相关协议//////////////////////////
//获取HLS播放地址请求
#define _DEF_PACK_GET_HLS_URL_RQ       (_DEF_PACK_BASE + 30 )
//获取HLS播放地址回复
#define _DEF_PACK_GET_HLS_URL_RS       (_DEF_PACK_BASE + 31 )

//转码状态定义
#define transcode_not_start     0   //未转码
#define transcode_processing    1   //转码中
#define transcode_done          2   //转码完成
#define transcode_failed        3   //转码失败

//获取HLS URL请求
struct STRU_GET_HLS_URL_RQ
{
    STRU_GET_HLS_URL_RQ():type(_DEF_PACK_GET_HLS_URL_RQ), userid(0), fileid(0){
        memset( dir, 0, sizeof(dir) );
    }
    PackType type;
    int userid;
    int fileid; //文件id
    char dir[_MAX_PATH_SIZE]; //文件所在目录
};

//获取HLS URL回复
struct STRU_GET_HLS_URL_RS
{
    STRU_GET_HLS_URL_RS():type(_DEF_PACK_GET_HLS_URL_RS), result(0), status(0){
    ...
    int result; //0失败，1成功
    int status; //转码状态
    char url[_MAX_PATH_SIZE]; //HLS播放地址（m3u8 URL）
    char dir[_MAX_PATH_SIZE]; //目录
};
